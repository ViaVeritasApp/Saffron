...
describe("HTML parser", function () {
    it('Test 1', function () {
        return getApp().scrape(JSON.parse(fs.readFileSync('./test/sources/html/html1.json', 'utf-8'))).then(articles => {
            ...
            for (const article of articles) {
                ...
                const cat = article.extra['__categories'].find((cat) => cat.name === 'Γενικές Ανακοινώσεις');
                expect(cat).to.not.be.undefined;
                expect(cat.links).to.deep.equal(['http://127.0.0.1:3000/html1']);
            }
            const article = articles[0];
            expect(article.extra.__categories[0].name).to.deep.equal('Γενικές Ανακοινώσεις');
            ...
            expect(article.url).to.equal('/unipi/el/ανακοινώσεις/item/13591-εις-μνήμην-χρυσούλας-τόμπρου.html');
            expect(article.publication_date).to.equal('Τετάρτη, 14 Σεπτεμβρίου 2022 12:15');
    ...
    it('Test 2', function () {
        return getApp().scrape(JSON.parse(fs.readFileSync('./test/sources/html/html2.json', 'utf-8'))).then(articles => {
            ...
            expect(article.url).to.equal('/anakoinoseis_kai_ekdiloseis/proboli_anakoinosis/ektakti_anakoinosi_gia_tin_orkomosia_ptychioychon_ioynioy_ioylioy_2022_klim_b_18_10_2022/');
            expect(article.publication_date).to.equal('2022-10-14');
            expect(article.attachments.length).to.equal(0);
            expect(article.thumbnail_url).to.be.undefined;
        });
    });
    it('Test 3', function () {
        return getApp().scrape(JSON.parse(fs.readFileSync('./test/sources/html/html3.json', 'utf-8'))).then(articles => {
            ...
            const article = articles[0];
            expect(article.title).to.equal('Ηλεκτρονικη υποβολη διπλωματικων και ερευνητικων εργασιων στη Βιβλιοθηκη και το Αρχειο του ΤΑΜΠΠ, Οκτωβρίου - Νοεμβρίου 2022' + ' - ' + article.publication_date);
            ...
            expect(article.url).to.equal('/el/news/news/ilektroniki-ypovoli-diplomatikon-kai-ereynitikon-ergasion-sti-vivliothiki-kai-to-arheio-toy-tampp-6355.html');
            expect(article.publication_date).to.equal('31/10/2022');
            expect(article.attachments.length).to.equal(0);
            expect(article.thumbnail_url).to.be.undefined;
        });
    });
    it('Test 4', function () {
        return getApp().scrape(JSON.parse(fs.readFileSync('./test/sources/html/html4.json', 'utf-8'))).then(articles => {
            ...
            expect(article.url).to.equal('/general_news/1108/anaboli_mathimatos_ydrodynamiki_sxediasi_mikrwn_skafwn');
            expect(article.publication_date).to.equal('Friday, 04 Νοεμβρίου 2022 - Γενικές Ανακοινώσεις');
            expect(article.attachments.length).to.equal(0);
            expect(article.thumbnail_url).to.be.undefined;
        });
    });
    it('Option: skip', function () {
        return getApp().scrape(JSON.parse(fs.readFileSync('./test/sources/html/html5.json', 'utf-8'))).then(articles => {
            ...
            for (const article of articles) {
                ...
                expect(article.publication_date).to.not.equal("Tuesday, 25 Οκτωβρίου 2022 - Ανακοινώσεις Προπτυχιακών");
                expect(article.title).to.not.equal("Αναπληρώσεις του μαθήματος Μηχανουργικές Κατεργασίες");
                expect(article.title).to.not.equal("ΠΡΟΓΡΑΜΜΑ ΚΛΕΙΣΤΗΣ ΑΙΘΟΥΣΑΣ ΑΘΛΟΠΑΙΔΙΩΝ 2022 - 2023");
            }
        });
    });
});
//# sourceMappingURL=parsers.html.test.js.map