...
describe("RSS parser", function () {
    ...
    it('Test 2', function () {
        return getApp().scrape(JSON.parse(fs.readFileSync('./test/sources/rss/rss2.json', 'utf-8'))).then(articles => {
            ...
            expect(article.url).to.equal('https://www.aspete.gr/04-11-2002-prosklisi-ekdilosis-endiaferontos-gia-tin-anadeixi-anadochoy-gia-tin-promitheia-ilektronikon-ypologiston-kai-ektypoton-gia-tin-kalypsi-anagkon-tis-aspaite-sto-maroysi-sto-etos-2022/');
            expect(article.publication_date).to.equal('Fri, 04 Nov 2022 12:40:19 +0000');
    ...
    it('Test 3', function () {
        return getApp().scrape(JSON.parse(fs.readFileSync('./test/sources/rss/rss3.json', 'utf-8'))).then(articles => {
            ...
            expect(article.url).to.equal('http://mbatqm.unipi.gr/%ce%bd%ce%ad%ce%b1-y%cf%80%ce%bf%ce%b2%ce%bf%ce%bb%ce%ae-%ce%b1%ce%b9%cf%84%ce%ae%cf%83%ce%b5%cf%89%ce%bd-%ce%b1%cf%80%cf%8c-22-8-2022-%ce%bc%ce%ad%cf%87%cf%81%ce%b9-%ce%ba%ce%b1%ce%b9-5-9-2022/');
            expect(article.publication_date).to.equal('Mon, 22 Aug 2022 08:27:59 +0000');
            expect(article.categories.length).to.equal(1);
            ...
            expect(Object.keys(article.extra).length).to.equal(2);
            expect(article.extra['dc:creator']).to.equal('my-email@gmail.com');
            expect(article.extra['test']).to.equal('Νέα Yποβολή αιτήσεων από 22/8/2022 μέχρι και 5/9/2022');
            expect(article.thumbnail_url).to.be.undefined;
        });
    });
});
//# sourceMappingURL=parsers.rss.test.js.map