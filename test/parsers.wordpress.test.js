...
describe("WordPress parser", function () {
    it('Test 1', function () {
        return getApp().scrape(JSON.parse(fs.readFileSync('./test/sources/wordpress-v2/wordpress1.json', 'utf-8'))).then(articles => {
            ...
            expect(article.url).to.equal('https://www.ds.unipi.gr/2022/09/15/intelab_anazitisi_ekpaideytikou_sept_2022/');
            ...
            expect(article.publication_date).to.equal('2022-09-15T11:08:05');
    ...
    it('Test 2', function () {
        return getApp().scrape(JSON.parse(fs.readFileSync('./test/sources/wordpress-v2/wordpress2.json', 'utf-8'))).then(articles => {
            ...
            expect(article.url).to.equal('https://financeclub.unipi.gr/2022/11/03/%ce%b1%cf%80%ce%bf%ce%bb%ce%bf%ce%b3%ce%b9%cf%83%ce%bc%cf%8c%cf%82-%cf%84%ce%b7%cf%82-%ce%b4%cf%81%ce%ac%cf%83%ce%b7%cf%82-%cf%84%ce%b7%cf%82-%ce%b1%cf%80%ce%b5%ce%bb%ce%b8%ce%bf%cf%8d%cf%83%ce%b1/');
            expect(article.categories.length).to.equal(1);
            ...
            expect(article.publication_date).to.equal('2022-11-03T21:19:47');
            expect(article.attachments.length).to.equal(1);
            ...
            expect(article.attachments[0].attribute).to.equal('href');
            expect(article.thumbnail_url).to.be.undefined;
            expect(articles[1].thumbnail_url).to.equal('https://financeclub.unipi.gr/wp-content/uploads/2022/09/cfa-opengraph-share-image-768x401.png');
        });
    });
});
//# sourceMappingURL=parsers.wordpress.test.js.map