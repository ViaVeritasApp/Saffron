...
import {extractLinks, isSkippedHTMLNode, parseHTMLField} from "../utils/html.js";

export type HtmlSkipOptions = {
    selector?: string;
    attribute?: string;
    text?: string;
...
export class HTMLParser extends Parser<HTMLInstructions> {
    static id = 'html';

    validate(s?: any): void {
        // This exists only for typescript, it is not valid and will not run at runtime.
        const scrape = s as HTMLInstructions;
        ...
        if (typeof scrape.scripting_enabled !== 'undefined' && typeof scrape.scripting_enabled !== 'boolean')
        ...
        for (const index in scrape.skip ?? []) {
            ...
            const {
                selector, text, attribute, type,
                position
            } = (<any>item);

            if(keys.includes("attribute") && !keys.includes("text")) {
                throw new Error(`skip.${index}.attribute must be together with "text"`);
            }

            if (keys.includes('selector') || keys.includes('text') || keys.includes("attribute")) {
                if (keys.includes('position'))
                    throw new Error(`skip.${index}.position cannot be together with "selector". "text" ot "attribute"`);
                ...
                if (attribute !== undefined && typeof attribute !== 'string')
                    throw new Error(`skip.${index}.attribute must be a string`);
    ...
    async parse(source: Source<HTMLInstructions>, response: AxiosResponse): Promise<Article[]> {
        const instructions = source.instructions;

        const articles: Article[] = [];
        const cheerioLoad = cheerio.load(response.data, {
            scriptingEnabled: instructions.scripting_enabled,
        });

        cheerioLoad(instructions.container).each((index, element) => {
            if (articles.length >= source.options?.articles?.amount!) return;
            if (isSkippedHTMLNode(instructions.skip, cheerioLoad, element, index)) return;
            ...
            article.title = parseHTMLField(articleData.title, true);
            article.content = parseHTMLField(articleData.content, false);
            article.url = parseHTMLField(articleData.url, false);
            article.publication_date = parseHTMLField(articleData.publication_date, false);
            article.thumbnail_url = parseHTMLField(articleData.thumbnail_url, false);

            article.author_name = parseHTMLField(articleData.author_name, true);
            article.author_image_url = parseHTMLField(articleData.url, false);
                ...
                if (Array.isArray(articleData.categories)) {
                    for(const c of articleData.categories) {
                        article.categories!.push({
                            name: cleanupHTMLText(c, true),
                            links: [this.url]
                        });
                    }
                } else {
                    article.categories!.push({
                        name: cleanupHTMLText(articleData.categories, true),
                        links: [this.url]
                    });
                }
            }
            ...
            Object.entries(articleData).forEach(extra => {
                if (HTMLParser.BASIC_DATA.includes(extra[0] as any) || !extra[1]) return;
    ...
    private attributes(location: cheerio.Cheerio<any>, attributesArr: string[]): HTMLAttribute[] {
    ...
    private getData(htmlContent: cheerio.CheerioAPI, currArticle: cheerio.BasicAcceptedElems<any>,
